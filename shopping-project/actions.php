...
if (isset($_POST['proceedToCartRemove'])) {
    if (!session_id()) {
        session_start();
    }
    if (!empty($_SESSION['outOfStrok']) || !isset($_SESSION['outOfStrok'])) {
        setcookie('user_cart', $u_cart, time() - (1000), '/', '', '', TRUE);
        setcookie('cart_count', $cart_count, time() - (1000), '/', '', '', TRUE);
    }
}
...
if (isset($_POST['proceedToCartRemoveWishlistItem'])) {
    setcookie('user_wishlist', $u_wishlist, time() - (180), '/', '', '', TRUE);
    setcookie('wishlist_count', $wishlist_count, time() - (180), '/', '', '', TRUE);
}
...
if (isset($_SESSION['user_id']) && isset($_POST['adlogout'])) {
    $uid = $_SESSION['user_id'];
    $db = new Database();
    $db->select('user', 'user_role', null, "user_id = $uid and user_role = 1");
    $result = $db->getResult();
    if ($result) {
        echo count($result);
    } else {
