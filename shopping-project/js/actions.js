...
// proceedToChechout remove-cart-item
$('#proceedToChechout').click(function(){
  //  let p_id = $('.remove-cart-item').attr('data-id');
  let p_id = 1;
   $.ajax({
            url: 'actions.php',
            method: 'POST',
            data: {proceedToCartRemove:p_id},
            success: function(data){
                location.reload();
            }
        });
...
//proceed-to-cart remove-wishlist-item
    $('#removeWishlistItemFromProceedToCart').click(function(){
        // let p_id = $('.remove-wishlist-item').attr('data-id');
        let wishlistID = 1;
        $.ajax({
            url: 'actions.php',
            method: 'POST',
            data: {proceedToCartRemoveWishlistItem:wishlistID},
            success: function(data){
                location.reload();
            }
        });
    ...
    $('#register_sign_up').submit(function(e){
        ...
        // if (f_name == '' || l_name == '' || email == '' || password == '' || mobile == '' || address == '' || city == ''){
    ...
    $('#modify-password').submit(function(e){
       ...
       $(document).ready(function() {
            $(document).on('click', '#ntfcount', function() {
                let count = 1;
                $.ajax({
